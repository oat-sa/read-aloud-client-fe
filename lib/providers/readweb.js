...
export default function readwebProvider(config = {}) {
    ...
    function createHighlightedElementObserver(targetNode) {
        ...
        highlightedElementObserver = new MutationObserver(mutationList => {
            mutationList
                .filter(
                    mutation =>
                        mutation.type === 'attributes' &&
                        mutation.attributeName === 'class' &&
                        mutation.target.classList.contains(highlightClass)
                )
                .forEach(mutation => {
        ...
        highlightedElementObserver.observe(targetNode, { attributes: true, subtree: true });
    }
