...
function swapLangAttribute(element) {
    ...
    if (langHolderElement) {
        ...
        if (langValue && langSubPart) {
            // include all children with the same lang attribute
            const children = langHolderElement.querySelectorAll(`[lang="${langValue}"]`);
            const langElements = [langHolderElement, ...children];
            for (let elt of langElements) {
