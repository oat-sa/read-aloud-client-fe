...
describe('readweb provider', () => {
    ...
    const readwebEntrypointV2 = jest.fn();
    const readwebEntrypointV3 = jest.fn();

    function mockReadwebScript({ apiVersion = 3 } = {}) {
        ...
        if (apiVersion === 2) {
            window.startleseweb = readwebEntrypointV2;
        } else if (apiVersion === 3) {
            window.vFact3 = {
                startReadWeb: readwebEntrypointV3
            };
        }
    ...
    it('loads the readweb script (v2 API)', () => {
        expect(document.querySelectorAll('script[data-tts="readweb"]')).toHaveLength(0);

        return Promise.all([
            getReadAloudClient('readweb', { url: './abc', license: 'foo' }),
            mockReadwebScript({ apiVersion: 2 })
        ]).then(results => {
            const script = document.querySelector('script[data-tts="readweb"]');
            ...
            expect(readwebEntrypointV2).toHaveBeenCalledTimes(1);
            expect(readwebEntrypointV3).not.toHaveBeenCalled();
        });
    });

    it('loads the readweb script (v2 API) in dev mode', () => {
        expect(document.querySelectorAll('script[data-tts="readweb"]')).toHaveLength(0);
    ...
    it('loads the readweb script (v3 API)', () => {
        expect(document.querySelectorAll('script[data-tts="readweb"]')).toHaveLength(0);

        return Promise.all([
            getReadAloudClient('readweb', { url: './abc', license: 'foo' }),
            mockReadwebScript({ apiVersion: 3 })
        ]).then(results => {
            const script = document.querySelector('script[data-tts="readweb"]');
            expect(script.src).toContain('/abc/foo.js');
            expect(script.dataset.loaded).toBeTruthy();
            ...
            expect(readwebEntrypointV2).not.toHaveBeenCalled();
            expect(readwebEntrypointV3).toHaveBeenCalledTimes(1);
        });
    });
    ...
    it('pauses', () =>
        Promise.all([getReadAloudClient('readweb', { url: './abc', license: 'foo' }), mockReadwebScript()]).then(
            results => {
                ...
                window.vFact3 = { pause: pauseHandler };
    ...
    it('resumes', () =>
        Promise.all([getReadAloudClient('readweb', { url: './abc', license: 'foo' }), mockReadwebScript()]).then(
            results => {
                ...
                window.vFact3 = { pause: resumeHandler }; // same call pauses & resumes
    ...
    it('ignore configured elements', () =>
        Promise.all([
            getReadAloudClient('readweb', { url: './abc', license: 'foo', ignoreSelector: '.second-ignore' }),
            mockReadwebScript()
        ]).then(results => {
            const client = results[0];

            container.innerHTML =
                '<p>Some text to <span class="ignore-me">ignore</span> read <span class="second-ignore">second ignore</span></p>';
            ...
            expect(container.querySelectorAll('.vFact_DoNotReadAloud')).toHaveLength(2);
        }));
    ...
    it('swap and restore the lang attribute', () => {
        ...
        return Promise.all([getReadAloudClient('readweb', { url: './abc', license: 'foo' }), mockReadwebScript()]).then(
            results => {
                ...
                container.innerHTML = `
                    <main>
                        <section lang="nb_NO">
                            <div>
                                <p>Some text</p>
                                <p lang="fr-BE">Un petit peu de <span lang="nb_NO">Norsk</span>.</p>
                            </div>
                        </section>
                    </main>`;
                ...
                const para2 = container.querySelector('p + p');
                const span = container.querySelector('span');

                expect(section.getAttribute('lang')).toEqual('nb_NO');
                expect(section.dataset.ttsOriginalLang).toBeFalsy();
                expect(para2.getAttribute('lang')).toEqual('fr-BE');
                expect(para2.dataset.ttsOriginalLang).toBeFalsy();
                expect(span.getAttribute('lang')).toEqual('nb_NO');
                expect(span.dataset.ttsOriginalLang).toBeFalsy();

                client.play(div);

                expect(section.getAttribute('lang')).toEqual('no');
                expect(section.dataset.ttsOriginalLang).toEqual('nb_NO');
                expect(para2.getAttribute('lang')).toEqual('fr');
                expect(para2.dataset.ttsOriginalLang).toEqual('fr-BE');
                expect(span.getAttribute('lang')).toEqual('no');
                expect(span.dataset.ttsOriginalLang).toEqual('nb_NO');
    ...
    it('autoscroll during play', () => {
        ...
        return Promise.all([getReadAloudClient('readweb', { url: './abc', license: 'foo' }), mockReadwebScript()])
            .then(results => {
                const client = results[0];
