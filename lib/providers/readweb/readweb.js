 ...
 */
import { speeds, pitches, voices } from '../../preferences.js';
...
import { createScrollTargetObserver } from '../../util/autoscroll.js';
...
const highlightClass = 'vFact_highlighted'; // -> 'vFact_highlighted_to_read' OR 'vFact_sentence_highlighted'
    ...
    return Promise.race([
        new Promise((resolve, reject) => {
            ...
            script.addEventListener('load', () => {
                if ('startleseweb' in window) {
                ...
                } else if ('vFact3' in window) {
                    // API v3 entry point
                    const vfactInit = {
                        PosRelativeTo: containerId
                    };
                    window.vFact3?.startReadWeb?.(vfactInit);
                    // cannot resolve() here as it's too early
                    // use polling because it can take 300-600ms before we have it

                    iframeContentInterval = setInterval(() => {
                        const frame = document.getElementById('vFact_audioFrame');
                        if (frame && 'vFactApi3' in frame.contentWindow) {
                            clearInterval(iframeContentInterval);
                    ...
                    reject(new Error(`Unable to find the readweb entrypoint 'startleseweb' / 'vFact3'`));
                }
            });
...
function createToolBarContainer() {
    if (!document.getElementById(containerId)) {
        ...
        topContainer.style.left = '-10000px';
        topContainer.style.top = 'auto';
        topContainer.style.width = '1px';
 ...
 */
function swapLangAttributes(element) {
    const langHolderElement = findClosestElementWithALang(element);
    const children = langHolderElement ? langHolderElement.querySelectorAll('[lang]') : [];
    for (const elt of [langHolderElement, ...children]) {
        if (elt) {
            const langValue = elt.getAttribute('lang');
            const langSubPart = getLanguagePart(langValue);
            if (langValue && langSubPart) {
                elt.dataset.ttsOriginalLang = langValue;
                elt.setAttribute('lang', langMap[langSubPart] ? langMap[langSubPart] : langSubPart);
            }
        }
    }
}
...
export default function readwebProvider(config = {}) {
    ...
    let readEndHandler;
    let highlightedElementObserver;
    ...
    /**
     * Creates an observer to follow the highlighted element and scroll the viewport to it
     * @param {HTMLElement} targetNode
     */
    function createHighlightedElementObserver(targetNode) {
        if (highlightedElementObserver) {
            destroyHighlightedElementObserver();
        }
        highlightedElementObserver = createScrollTargetObserver(targetNode, highlightClass);
    }

    /**
     * Destroys highlightedElementObserver
     */
    function destroyHighlightedElementObserver() {
        if (highlightedElementObserver) {
            highlightedElementObserver.disconnect();
            highlightedElementObserver = null;
        }
    }
        ...
        return {
            ...
            play(element) {
                if (element instanceof HTMLElement && typeof window.vFact_playsection === 'function') {
                    ...
                    swapLangAttributes(element);

                    window.vFact_playsection(element.id);

                    if (this.autoscroll) {
                        createHighlightedElementObserver(element);
                    }
                }
            },
            ...
            playSelection() {
                if (typeof window.vFact_doplay === 'function' && isTextSelected()) {
                    ...
                    swapLangAttributes(ancestorElement);

                    window.vFact_doplay();

                    if (this.autoscroll) {
                        createHighlightedElementObserver(ancestorElement);
                    }
                }
            },
             ...
             */
            pause() {
                if (typeof window.vFact3?.pause === 'function') {
                    window.vFact3.pause();
                }
            },
             ...
             * @param {string} preferences.voice - from the available vocies values
             * @param {boolean} [preferences.autoscroll] - scroll viewport to element
             */
            setPreferences({ speed, pitch, voice, autoscroll } = {}) {
                if (typeof window.vfact_SetCustomParams === 'function') {
                    window.vfact_SetCustomParams(
                        ...
                        void 0, // this is an unused parameter by the library
                        typeof pitchMap[pitch] === 'number' ? pitchMap[pitch] : pitchMap[pitches.medium]
                    );
                }
