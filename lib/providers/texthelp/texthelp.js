...
import { updateOverridenVoices, lookupVoice } from './voices.js';
...
function loadSpeechStreamScript(url, speechstreamConfig, loadingTimeoutMs = 30000) {
    ...
    return Promise.race([
        new Promise((resolve, reject) => {
            ...
            // script src="https://toolbar.speechstream.net/SpeechStream/3.9.0/speechstream.js" type="text/javascript" data-speechstream-config="TAOPremiumv390R1"
 ...
 * @param {boolean} [config.disableLangCheck] - If false, the DOM will be checked for a lang on each play.
 *                                            - If true, SpeechStream only looks at the <html lang> to pick the voice. A default voiceLanguage should be specified.
...
export default function texthelpProvider(config = {}) {
    ...
    let ignoreSelector = config.ignoreSelector || '.do-not-read';
    ...
    return loadSpeechStreamScript(config.url, config.speechstreamConfig, config.loadingTimeoutMs).then(api => {
        ...
        /**
         * SpeechStream has their own `lang` attribute recognition flow:
         * 1. If getUseElementLangTags() === false, SpeechStream will look only at the <html lang> attribute to pick a voice.
         *    Therefore we make sure to change this attribute.
         * 2. If getUseElementLangTags() === true, SpeechStream has its own logic to pick the voice.
         *    However, we also influence this logic, using `setVoiceBasedOnElementLang`.
         */
        if (!allowLangCheck) {
            speechTools.setUseElementLangTags(false);
            const originalDocumentLang = document.documentElement.getAttribute('lang');
            if (originalDocumentLang) {
                document.documentElement.setAttribute('data-pre-texthelp-lang', originalDocumentLang);
            }
            document.documentElement.setAttribute('lang', voiceLanguage);
        }

        // Set the initial voice, as it may or may not be set again on play
        speechTools.setVoice(defaultVoice.name);
        ...
        function setVoiceBasedOnElementLang(element) {
            ...
            if (langHolderElement) {
                ...
                speechTools.setVoice(voice.name);
            }
        }
        ...
        return {
             ...
             * @param {Boolean} [options.andStop=false]
             */
            play(element, { andStop = false } = {}) {
                if (element instanceof HTMLElement && typeof speechTools.speakElement === 'function') {
                    ...
                    speechTools.speakElement(element, !andStop);
                }
            },
            ...
            ignoreElements(selector = '') {
                ...
                if (typeof domControl.addIgnoreListQuerySelector === 'function' && ignoreSelector) {
             ...
             * @param {string} [preferences.speed] - speed from the available speeds values
             * @param {string} [preferences.voice] - gender from the available voices values (modifies value initially read from config.voiceGender)
             ...
             * @param {Object} [preferences.voiceMapping] - to update our mapping of voices (NOT speechstream's internal one)
             *                                              Object { female: { [langCode]: voiceName, ... }, male: { [langCode]: voiceName, ... } }
             */
            setPreferences({ speed, voice, language, disableLangCheck, autoscroll, voiceMapping } = {}) {
                // apply any new voiceMapping before calling lookupVoice
                if (voiceMapping) {
                    ['female', 'male'].forEach(gender => {
                        for (const lang in voiceMapping[gender] || {}) {
                            if (voiceMapping[gender][lang]) {
                                updateOverridenVoices(gender, lang, voiceMapping[gender][lang]);
                            }
                        }
                    });
                }
             ...
             * @returns {import('../index.js).OptionConstraints}
             */
            getSupport() {
                /*
                ...
                return {
                    ...
                    speed: {
                        disabled: false,
                        options: [speeds.slowest, speeds.normal, speeds.fastest]
                    },
                    pitch: { disabled: true },
                    playSelection: false,
                    clickToSpeak: true,
            ...
            destroy() {
                ...
                const originalDocumentLang = document.documentElement.getAttribute('data-pre-texthelp-lang');
                if (originalDocumentLang) {
                    document.documentElement.setAttribute('lang', originalDocumentLang);
                    document.documentElement.removeAttribute('data-pre-texthelp-lang');
                }
            }
        };
    });
