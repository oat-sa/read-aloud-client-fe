...
/**
 * Set an override for a gender and locale
 * @param {String} gender
 * @param {String} locale
...
export function updateOverridenVoices(gender, locale, voiceName) {
    ...
    const foundVoice = defaultVoicesByLangGender[gender][locale]
        || altVoicesByLangGender[gender][locale]?.find(voice => voice.name === voiceName);

    if (foundVoice) {
        overridenVoicesByLangGender[gender][locale] = foundVoice;
        console.log('updateLanguageToVoiceByGenderOverrides', [gender, locale, voiceName]);
        return true;
    }
    return false;
}
...
export function resetOverridenVoices(gender, locale) {
    if (gender && locale && locale in overridenVoicesByLangGender[gender]) {
        overridenVoicesByLangGender[gender][locale] = defaultVoicesByLangGender[gender][locale];
        return;
    }
