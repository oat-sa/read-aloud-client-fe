    ...
    it.each([
        ...
        Promise.all([getReadAloudClient('texthelp', { url, speechstreamConfig }), mockTexthelpScript()]).then(
            results => {
                ...
                client.play(element);
                expect(speechstreamApiMock.speechTools.setVoice).toHaveBeenCalledWith(expect.objectContaining({ name: expectedVoice }));
    ...
    it('sets preferences', () =>
        Promise.all([getReadAloudClient('texthelp', { url, speechstreamConfig }), mockTexthelpScript()]).then(
            results => {
                ...
                expect(speechstreamApiMock.speechTools.setVoice).toHaveBeenLastCalledWith(expect.objectContaining({ name: 'Thomas'} ));
    ...
    it('calls setVoice only once if preferences set with disableLangCheck: true', () =>
        Promise.all([getReadAloudClient('texthelp', { url, speechstreamConfig }), mockTexthelpScript()]).then(
            results => {
                ...
                expect(speechstreamApiMock.speechTools.setVoice).toHaveBeenLastCalledWith(expect.objectContaining({ name: 'Yannick'} ));
