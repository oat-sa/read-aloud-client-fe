...
describe('texthelp provider', () => {
    ...
    it('sets preferences', () =>
        Promise.all([getReadAloudClient('texthelp', { url, speechstreamConfig }), mockTexthelpScript()]).then(
            results => {
                ...
                expect(speechstreamApiMock.speechTools.setVoiceSpeed).toHaveBeenCalledWith(40);
    ...
    it('calls setVoice only once if preferences set with disableLangCheck: true', () =>
        Promise.all([getReadAloudClient('texthelp', { url, speechstreamConfig }), mockTexthelpScript()]).then(
            results => {
                ...
                expect(speechstreamApiMock.speechTools.setVoice).toHaveBeenLastCalledWith('Yannick');
    ...
    it('sets "pron" attribute from "data-tts-text" attribute', () =>
        Promise.all([getReadAloudClient('texthelp', { url, speechstreamConfig }), mockTexthelpScript()]).then(
            results => {
                const client = results[0];
                ...
                container.innerHTML =
                    '<p>Size is <span data-tts-text="twelve">12</span> or more <span data-tts-text="meters">m.</span></p>';

                // play, override for children
                client.play(container.querySelector('p'));
                expect(container.innerHTML).toBe(
                    '<p>Size is <span data-tts-text="twelve" pron="twelve">12</span> or more <span data-tts-text="meters" pron="meters">m.</span></p>'
                );

                // play again after new child was added
                const newChild = document.createElement('span');
                newChild.innerHTML = 'ne';
                newChild.setAttribute('data-tts-text', 'new elem');
                container.querySelector('p').appendChild(newChild);
                client.play(container.querySelector('p'));
                expect(container.innerHTML).toBe(
                    '<p>Size is <span data-tts-text="twelve" pron="twelve">12</span> or more <span data-tts-text="meters" pron="meters">m.</span>' +
                        '<span data-tts-text="new elem" pron="new elem">ne</span></p>'
                );

                // play, override for element itself
                container.innerHTML = '<p data-tts-text="et cetera">etc</p>';
                client.play(container.querySelector('p'));
                expect(container.innerHTML).toBe('<p data-tts-text="et cetera" pron="et cetera">etc</p>');

                // playSelection, override for children
                container.innerHTML =
                    '<p>Size is <span data-tts-text="twelve">12</span> <em>or more <span data-tts-text="meters">m.</span></em></p>';
                window.getSelection = jest.fn(() => ({
                    rangeCount: 1,
                    isCollapsed: false,
                    getRangeAt: () => ({
                        commonAncestorContainer: container.querySelector('p em')
                    })
                }));
                client.playSelection();
                container.innerHTML =
                    '<p>Size is <span data-tts-text="twelve">12</span> <em>or more <span data-tts-text="meters" pron="meters">m.</span></em></p>';
