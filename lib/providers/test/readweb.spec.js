    ...
    it('set preferences', () =>
        Promise.all([getReadAloudClient('readweb', { url: './abc', license: 'foo' }), mockReadwebScript()]).then(
            results => {
                ...
                client.setPreferences('slow', 'highest', 'medium', 'M');

                expect(paramHandler).toHaveBeenNthCalledWith(1, 5, 'M', 1);

                client.setPreferences('foo', '12', 'medium', 'aVoice');

                expect(paramHandler).toHaveBeenNthCalledWith(2, 2, 'F', 2);
            }
        ));
});
