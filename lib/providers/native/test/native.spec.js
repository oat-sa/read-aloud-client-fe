...
describe('native provider', () => {
    ...
    it('plays from a dom element', () => {
        ...
        return getReadAloudClient('native', { ignoreSelector: '.ignore-me' }).then(client => {
            ...
            expect(window.speechSynthesis.speak).toHaveBeenCalledWith(
                expect.objectContaining({ text: 'Some text to read' })
            );
        });
    });
    ...
    it('plays from a selection', () => {
        ...
        return getReadAloudClient('native').then(client => {
            ...
            expect(window.speechSynthesis.speak).toHaveBeenCalledWith(
                expect.objectContaining({ text: 'selected text to read' })
            );
        });
    });
    ...
    it('pauses and resumes', () => {
        ...
        return getReadAloudClient('native').then(client => {
            ...
            const element = container.querySelector('p');
            element.innerText = 'Some text to read';
            ...
            client.pause();
            expect(window.speechSynthesis.pause).toHaveBeenCalledTimes(2);
            window.speechSynthesis.speaking = false;
