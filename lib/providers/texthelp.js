...
import { findClosestElementWithALang, normalize, getLanguagePart } from '../util/locale.js';
    ...
    return loadSpeechStreamScript(config.url, config.speechstreamConfig, config.loadingTimeoutMs).then(api => {
        ...
        function setVoiceBasedOnElementLang(element) {
            ...
            if (langHolderElement) {
                ...
                const lowerLangValue = normalize(langValue).toLowerCase();
                const lowerLangPrefix = getLanguagePart(lowerLangValue);

                const langVoiceMap = languageToVoiceByGender[voiceGender];

                if (langValue && lowerLangValue in langVoiceMap) {
                    speechTools.setVoice(langVoiceMap[lowerLangValue]);
                } else if (lowerLangPrefix && lowerLangPrefix in langVoiceMap) {
                    speechTools.setVoice(langVoiceMap[lowerLangPrefix]);
                }
            }
        }
        ...
        return {
            ...
            setPreferences({ speed, voice, autoscroll, highlightStyle } = {}) {
                ...
                if (voice) {
                    voiceGender = voiceMap[voice] || voiceMap[voices.female];
                }
