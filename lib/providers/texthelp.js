...
export default function texthelpProvider(config = {}) {
    ...
    const defaultVoice = lookupVoice(config.voiceGender, config.voiceLanguage);
    const allowLangCheck = !config.disableLangCheck;
    ...
    return loadSpeechStreamScript(config.url, config.speechstreamConfig, config.loadingTimeoutMs).then(api => {
        ...
        // Disable the library's check of lang attrs, because we have our own implementation
        ...
        function setVoiceBasedOnElementLang(element) {
            ...
            if (langHolderElement) {
                speechTools.setVoice(lookupVoice(voiceGender, langHolderElement.getAttribute('lang')));
            }
        }
        ...
        return {
            ...
            toggleClickToSpeak() {
                if (
                    ...
                    speechTools.setUseElementLangTags(allowLangCheck && newCtsState);
                    return newCtsState;
                }
                return false;
            },
