...
export default function texthelpProvider(config = {}) {
    ...
    return loadSpeechStreamScript(config.url, config.speechstreamConfig, config.loadingTimeoutMs).then(api => {
        ...
        return {
             ...
             */
            toggleClickToSpeak() {
                if (typeof speechTools.clickToSpeak === 'function' && typeof speechTools.getClickToSpeakState === 'function') {
                    speechTools.clickToSpeak();
                    return speechTools.getClickToSpeakState();
                }
                return false;
            },
 ...
 * It should be applied as a HTTP response header or a <meta> tag, before the content to be read loads.
...
/**
 * Injects a <meta> tag with the TextHelp Content Security Policy
 */
export function installCSP() {
    const metaTag = document.createElement('meta');
    metaTag.setAttribute('http-equiv', 'Content-Security-Policy');
    metaTag.setAttribute('content', contentSecurityPolicy.replace(/\s+/g, ' '));
