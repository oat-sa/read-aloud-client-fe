 ...
 */
import { speeds, voices, pronunciationAttributeName } from '../preferences.js';
...
export default function texthelpProvider(config = {}) {
    ...
    return loadSpeechStreamScript(config.url, config.speechstreamConfig, config.loadingTimeoutMs).then(api => {
        ...
        /**
         * Configure text to read instead of element's html content, if override is defined.
         * Attribute for override in our markup is "data-tts-text", but TextHelp's is "pron", so should rename it.
         * @param {HTMLElement} element
         */
        function setPronunciationOverride(element) {
            if (element) {
                const providerAttr = 'pron';
                const lookupSelector = `[${pronunciationAttributeName}]:not([${providerAttr}])`;
                if (element.matches(lookupSelector)) {
                    element.setAttribute(providerAttr, element.getAttribute(pronunciationAttributeName));
                }
                element.querySelectorAll(lookupSelector).forEach(el => {
                    el.setAttribute(providerAttr, el.getAttribute(pronunciationAttributeName));
                });
            }
        }
        ...
        return {
            ...
            play(element) {
                if (element instanceof HTMLElement && typeof speechTools.speakElement === 'function') {
                    ...
                    setPronunciationOverride(element);
            ...
            playSelection() {
                if (isTextSelected()) {
                    try {
                        ...
                        setPronunciationOverride(ancestorElement);
